...
(meaning that the customer responded to the telemarketing offer i.e. `yes` and else `no` in the target column `y`).
...
Class imbalance is observed in the target class, it will be decided in hyper-parameter tuning in the next session to see
if balancing the class will leads to better model.
...
Moreover, we believe that including the reason for the last contact in the data gathering process would 
add valuable information and increase the accuracy of the model.

Lastly, we lack context on the campaign schedule and other environment variables, 
